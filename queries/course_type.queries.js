...
const getcourseTypeById = async (req, res) => {
  try {
    ...
    return res.status(200).json(getcourseType);
  } catch (error) {
...
const updatecourseType = async (req, res) => {
  try {
    ...
    //    }
    const id = req.params.id;
    const updatecourseType = await Course_type.update(req.body, {
      where: {
        id: id,
      },
    });
...
const getCoursesBycourseType = async (req, res) => {
  try {
    ...
    const getCourseType = await Course.findAll(
      { include: [{ model: Course_type, as: "Course_type" }] },
      {
        where: {
          type_name: TYPE_NAME,
        },
